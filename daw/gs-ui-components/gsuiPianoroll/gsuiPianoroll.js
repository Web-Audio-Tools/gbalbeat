...
class gsuiPianoroll extends HTMLElement {
	...
	addKey( id, obj ) {
		...
		this.#blcManager.__blcs.set( id, blc );
		obj.selected
	...
	removeKey( id ) {
		const blc = this.#blcManager.__blcs.get( id ),
			blcPrev = this.#blcManager.__blcs.get( blc.dataset.prev );
	...
	#blockDOMChange( el, prop, val ) {
		switch ( prop ) {
			...
			case "prev": {
				const blc = this.#blcManager.__blcs.get( val );
	...
	#blockRedrawDragline( el ) {
		const blcPrev = this.#blcManager.__blcs.get( el.dataset.prev );
	...
	#ongsuiTimewindowPxperbeat( ppb ) {
		...
		this.#blcManager.__blcs.forEach( blc => blc._dragline.redraw() );
	...
	#rowMousedown( key, e ) {
		...
		if ( e.button === 0 && !e.shiftKey ) {
			const when = this.#blcManager.__roundBeat( this.#blcManager.__getWhenByPageX( e.pageX ) );
	...
	#onchangeSlidersSelect() {
		...
		this.#blcManager.__blcs.forEach( ( blc, id ) => {
	...
	// ........................................................................
	#getDropAreas( id ) {
		const d = this.#blcManager.__blcs.get( id ).dataset,
			when = +d.when + +d.duration,
			arr = [];

		this.#blcManager.__blcs.forEach( blc => {
			const d = blc.dataset;
...
window.gsuiPianoroll = gsuiPianoroll;